...
public class SwitchView extends View {
    ...
    public SwitchView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < indexCount; i++) {
            ...
            if (attr == R.styleable.SwitchView_switch_bg_close_color) {
                switchViewBgCloseColor = typedArray.getColor(attr, Color.BLACK);

            } else if (attr == R.styleable.SwitchView_switch_bg_open_color) {
                switchViewBgOpenColor = typedArray.getColor(attr, Color.BLACK);

            } else if (attr == R.styleable.SwitchView_switch_bg_strock_width) {
                switchViewStrockWidth = typedArray.getDimensionPixelOffset(attr, 0);

            } else if (attr == R.styleable.SwitchView_switch_strock_color) {
    ...
    public void setOnCheck(OnCheck check){
        this.check = check;
    }
