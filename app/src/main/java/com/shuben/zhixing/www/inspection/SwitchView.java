...
public class SwitchView extends LinearLayout implements OnClickListener {
	...
	private int margin = 20; // 游标离边缘位置(这个值视图片而定, 主要是为了图片能显示正确)
...
//		iv_switch_cursor.setLayoutParams(new LayoutParams(iv_w,iv_h));
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		P.c(cursor_left+"=="+cursor_right+"=="+bg_left+"=="+bg_right);
	}
	...
	private void initView() {
		...
		View view = inflater.inflate(R.layout.switch_lay, this);
	...
	private void changeTextColor() {
		if (isChecked) {
            switch_text_true.setTextColor(getResources().getColor(R.color.title_bg));
            switch_text_false.setTextColor(Color.WHITE);

		} else {
            switch_text_true.setTextColor(Color.WHITE);
            switch_text_false.setTextColor(getResources().getColor(R.color.title_bg));
		}
	}
