package com.shuben.zhixing.module.mass.chart.wx;
 ...
 * 不良率产品型号柏拉图
 */
public class WX_Chart2 extends BaseActvity {
    ...
    public void process(Message msg) {
            switch (msg.what){
                case 0:
                    ...
                    ShowMassPie pie = new ShowMassPie(WX_Chart2.this,si,"0","1",startTime,endTime,type);
                ...
                case 1:
                    ...
                    if(situationArrt.size()!=0){
                        combineChartManager = new CombinedChartManager(WX_Chart2.this,getHandler(),mChart);
    ...
    public void initLayout() {
        ...
        select_time.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ChangeDoubleTime doubleTime = new ChangeDoubleTime(WX_Chart2.this);
   ...
   /* private void test(){
    ...
    private void parse(JSONObject jsonObject){
        ...
        try {
            ...
            Map<String,Integer> temp = new HashMap<>();//统计所有报废和维修不成功
            Map<String,Integer> tempTotal = new HashMap<>();//统计所有
            float total = 0;
            for(int i=0;i<len;i++){
                JSONObject o = jsonArray.getJSONObject(i);
                String key =  o.getString("ProductCode");
                int oo= o.getInt("RepairCount");
                String tip = o.getString("Result");

//                P.c(key+"==>"+oo+"=="+tip);
                if(temp.containsKey(key)){
                    //修改
//                           temp.get(key).add(oo);
                    if(tip.equals("报废")||tip.equals("维修不成功")){
                        temp.put(key,temp.get(key)+oo);
                        total = total+oo;
                    }
                    tempTotal.put(key,tempTotal.get(key)+oo);
            ...
            while(it.hasNext()){
                String key  =   it.next();
                float c = temp.get(key).floatValue();
                float t = tempTotal.get(key).floatValue();

               // P.c(key+"---"+c+"==="+t);
                WxBean b = new WxBean();
                b.setName(key);
                b.setCount(temp.get(key));//不良计数
                if(c==0){
                    //c有可能为0
                    b.setVal(0);
                }else{
            ...
            //排序
//                    int sel = 1;
            Collections.sort(its,(wxBean, t1) -> {
                if(wxBean.getVal()>t1.getVal()){
                    return -1;
                }
                return 1;
            });
    ...
    private  void loadData(){
        ...
        params.put("productCode",edit.getTextEx().intern());
        params.put("picNO","");
        ...
        params.put("TenantId",SharedPreferencesTool.getMStool(WX_Chart2.this).getTenantId());
//        params.put("TenantId",SharedPreferencesTool.getMStool(context).getTenantId());
        httpPostVolley(SharedPreferencesTool.getMStool(WX_Chart2.this).getIp() + UrlUtil.Url, params, new VolleyResult() {
            @Override
