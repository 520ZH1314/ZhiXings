...
public class TransferImage extends FrameLayout {
    private String BASE_URL_IMAGE = "hhtp://";//地址
    ...
    private void loadImage(final int position) {
        ImageEntity entity = attr.getImageStrList().get(position);
        String imgUrl = entity.getImageUrl();
        Drawable placeHolder = getPlaceHolderDrawable(position);

        if (entity.isLocal()) {
            attr.getImageLoader().loadImage(new File(imgUrl), imagePagerAdapter.getImageItem(position), placeHolder);
        } else {
            attr.getImageLoader().loadImage(BASE_URL_IMAGE + imgUrl, imagePagerAdapter.getImageItem(position), placeHolder, new ImageLoader.Callback() {
...
//            int reHeight = getWidth() * intrinsicHeight / intrinsicWidth;
    ...
    public static class Builder {
        ...
        private ArrayList<ImageEntity> imageUrlList;
        ...
        public Builder setImageUrls(ArrayList<ImageEntity> imageUrls) {
            this.imageUrls = imageUrls;
            return this;
        }

        public Builder setImageUrlList(ArrayList<ImageEntity> imageUrlList) {
        ...
        public TransferImage setup(TransferImage transferImage) {
            ...
            if (imageUrlList != null && imageUrlList.size() > 0) {
                ...
                attr.setImageUrlList(imageUrls);
            }
